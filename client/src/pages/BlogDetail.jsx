      ...
      {post.image && (
        ...
        >
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            <span className="bg-black/50 text-white px-4 py-2 rounded-full text-sm backdrop-blur-md">View Image</span>
          </div>
        </motion.div>
      )}
        ...
        {post.tags?.length > 0 && (
          <div className="mt-12 pt-8 border-t border-gray-100 dark:border-white/5">
            ...
            <div className="flex flex-wrap gap-2">
              {post.tags.map((tag) => (
                <span key={tag} className="flex items-center gap-1.5 px-4 py-2 rounded-full bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-300 text-sm font-medium">
                  <Tag size={13} /> {tag}
                </span>
              ))}
            </div>
