...
export default function ProjectDetail() {
  ...
  if (loading || !project) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-black text-gray-500 dark:text-white/50">
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black text-gray-900 dark:text-white pb-24 pt-24 px-4 md:px-8">
      ...
      >
        ...
        <div className="flex flex-wrap gap-4 items-center text-sm text-gray-500 dark:text-white/50 mb-6">
          {project.createdAt && (
            <div className="flex items-center gap-1.5">
              ...
              <span>{new Date(project.createdAt).toLocaleDateString('en-US', { day: 'numeric', month: 'long', year: 'numeric' })}</span>
            </div>
          )}
          {project.category && (
            <div className="flex items-center gap-1.5">
        ...
        {/* Links */}
        {(project.links?.length > 0 || project.demoLink || project.repoLink) && (
          <div className="flex flex-wrap gap-3 mb-8">
            ...
            {!project.links?.length && (
              <>
                {project.demoLink && (
                  <a href={project.demoLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-purple-600 hover:bg-purple-700 text-white transition-all font-medium text-sm">
                    <ExternalLink size={15} /> Live Demo
                  </a>
                )}
                {project.repoLink && (
                  <a href={project.repoLink} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-2.5 rounded-full bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-all font-medium text-sm">
                    Source Code
                  </a>
                )}
              </>
          ...
          <div className="mt-10">
            ...
            <div className="flex flex-wrap gap-2">
              {project.techStack.map((tech) => (
                <span key={tech} className="px-4 py-2 rounded-full bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300 text-sm font-medium border border-gray-200/50 dark:border-white/5">
                  {tech}
                </span>
              ))}
            </div>
        ...
        {project.content && (
          <div className="mt-12 pt-10 border-t border-gray-100 dark:border-white/5">
            <div className="prose dark:prose-invert max-w-none prose-lg prose-purple prose-img:rounded-2xl" dangerouslySetInnerHTML={{ __html: project.content }} />
          </div>
        )}
