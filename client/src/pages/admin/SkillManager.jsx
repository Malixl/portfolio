...
import { Plus, Loader2, Trash2, Pencil, X, Zap, Code2, Wrench, Users } from 'lucide-react'
...
function SkillIcon({ icon }) {
  ...
  if (icon.startsWith('http')) {
    return <img src={icon} alt="" className="w-6 h-6 object-contain" onError={(e) => { e.target.style.display = 'none' }} />
  }
  return <span className="text-xl">{icon}</span>
}

function SkillForm({ onSubmit, onCancel, initial = null, defaultType = 'tech' }) {
  ...
  const typeConfig = SKILL_TYPES.find((t) => t.key === currentType) || SKILL_TYPES[0]
    ...
    <motion.div className="fixed inset-0 z-100 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <motion.div className="relative w-[90vw] max-w-md bg-white dark:bg-[#0d0d0d] border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl dark:shadow-none" initial={{ y: 30, scale: 0.95 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20, scale: 0.97 }}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-white/10">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">{isEdit ? 'Edit Skill' : `New ${typeConfig.label.slice(0, -1)}`}</h2>
        ...
        <form onSubmit={handleSubmit(onForm)} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input {...register('name', { required: 'Required' })} className="input-field" placeholder={isTech ? 'React.js' : currentType === 'hardskill' ? 'UI Design' : 'Problem Solving'} />
          ...
          {isTech && (
            <>
              <div>
                <label className="admin-label">Icon (emoji or image URL)</label>
                <input {...register('icon')} className="input-field" placeholder="⚛️ or https://cdn.example.com/icon.svg" />
                <p className="text-gray-400 dark:text-white/20 text-xs mt-1">Use emoji like ⚛️ or paste an image URL</p>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="admin-label">Category</label>
                  <Controller name="category" control={control} defaultValue="" render={({ field }) => (
                    <CustomSelect options={TECH_CATEGORIES} value={field.value} onChange={field.onChange} placeholder="Select..." />
                  )} />
                </div>
                <div>
                  <label className="admin-label">Level</label>
                  <Controller name="level" control={control} defaultValue="Intermediate" render={({ field }) => (
                    <CustomSelect options={PROFICIENCY_LEVELS} value={field.value} onChange={field.onChange} placeholder="Select..." />
                  )} />
                </div>
              </div>
            </>
          ...
          {!isTech && (
            <>
              <div>
                ...
                <textarea {...register('description')} className="input-field min-h-[80px] resize-none" placeholder="Brief description of this skill..." rows={3} />
              </div>
              <div>
                <label className="admin-label">Year Acquired</label>
                <input {...register('year')} type="number" min={2000} max={2030} className="input-field" placeholder="2023" />
              </div>
            </>
          )}
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button type="submit" disabled={submitting} className="flex items-center gap-2 bg-purple-600 hover:bg-purple-500 disabled:bg-purple-600/50 text-white text-sm font-medium px-5 py-2.5 rounded-lg transition-colors">
              {submitting ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : isEdit ? 'Save Changes' : 'Add Skill'}
            </button>
          </div>
        </form>
      </motion.div>
...
export default function SkillManager() {
  const { items: skills, loading, addItem, updateItem, deleteItem } = useCrud('/skills', 'Skill')
    ...
    <div className="p-4 md:p-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 dark:text-white/40 text-sm mt-1">{skills.length} skill{skills.length !== 1 ? 's' : ''} total</p>
        </div>
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2 no-scrollbar">
        {SKILL_TYPES.map((t) => {
          const Icon = t.icon
          const count = skills.filter((s) => (s.type || 'tech') === t.key).length
          ...
          return (
            ...
            >
              <Icon size={16} />
              {t.label}
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${active ? 'bg-purple-100 text-purple-600 dark:bg-purple-500/20 dark:text-purple-300' : 'bg-gray-100 text-gray-400 dark:bg-white/5 dark:text-white/20'}`}>{count}</span>
            </button>
          )
        })}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-white dark:bg-[#111] border border-gray-200 dark:border-white/5 rounded-xl">
          <Zap size={40} className="text-gray-300 dark:text-white/10 mx-auto mb-3" />
          <p className="text-gray-500 dark:text-white/30 text-lg">No {tabConfig?.label.toLowerCase()} yet</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filtered.map((skill) => (
            <div key={skill._id} className="bg-white dark:bg-[#111] border border-gray-200 dark:border-white/5 rounded-xl p-4 group hover:border-purple-500/30 dark:hover:border-white/10 hover:shadow-lg hover:shadow-gray-200/50 dark:hover:shadow-none transition-all">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="text-gray-900 dark:text-white font-medium text-sm truncate">{skill.name}</p>
                    {skill.type === 'tech' ? (
                      <span className="text-[10px] px-1.5 py-0.5 bg-purple-50 text-purple-600 dark:bg-purple-500/15 dark:text-purple-300 rounded-full">{skill.category || 'General'}</span>
                    ) : (
                      <span className="text-[10px] text-gray-400 dark:text-white/30">{skill.year ? `Since ${skill.year}` : ''}</span>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all shrink-0 ml-2">
                  <button onClick={() => openEdit(skill)} className="p-1.5 rounded-lg text-gray-400 hover:text-gray-900 hover:bg-gray-100 dark:text-white/40 dark:hover:text-white dark:hover:bg-white/10 transition-all" title="Edit">
                    <Pencil size={13} />
                  </button>
                  <button onClick={() => handleDelete(skill._id)} className="p-1.5 rounded-lg text-red-500/70 hover:text-red-600 hover:bg-red-500/10 dark:text-red-400/70 dark:hover:text-red-400 dark:hover:bg-red-500/15 transition-all" title="Delete">
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
              ...
              {skill.type !== 'tech' && skill.description && (
                <p className="text-gray-500 dark:text-white/30 text-xs mt-2 line-clamp-2">{skill.description}</p>
              )}
            </div>
          ))}
        </div>
